...
data <-read.spss(paste0(panel_data_path, file_of_interest),to.data.frame = T,use.value.labels = F,reencode = F)
...
# transform  factor levels to years (different for 2005 & 2010)
main <- main %>% mutate(yob = case_when(yob == 1 & wave_id == "2005" ~ 1900,
                        yob %in% c(2:88) & wave_id == "2005"  ~ 1900 + (yob -1),
                        yob  == 89 & wave_id == "2005"  ~ NA_real_,
                        
                        yob == 1 & wave_id == "2010" ~ 1900,
                        yob %in% c(2:93) & wave_id == "2010"  ~ 1900 + (yob -1),
                        yob  > 93 & wave_id == "2010"  ~ NA_real_))
...
#calculate age and remove yob information
...
# RISKGEN4: Reverse coding (higher score = higher risk taking)
main <- main %>%
  mutate(RISK_GEN_ORD4A = abs(RISK_GEN_ORD4A-5))
