...
panel_data_path <- c("~/Documents/TSRP/Data/NSHAP/RawData/") # where the raw panel data is stored
preproc_data_path <- c("~/Documents/TSRP/Data/NSHAP/ProcData/") # where to save processed panel data
panel_name <- "NSHAP"
...
for (CurrWave in wave_list) {
  ...
  data_file <- list.files(path = panel_data_path, pattern = paste0(unique(sub_all_info$varfile), "-Data.sav"), full.names = TRUE)
  
  wave_data <- read_sav(data_file) 
  
  # select vars of interest
  data <- wave_data %>% select(all_of(vars_of_interest)) 
  ...
  # place in list
  data_list[[wave_num]] <- data %>%  mutate(wave_id = CurrWave, date = as.character(date))
  wave_num <- wave_num + 1
} # wave loop
...
# For R3 & R2 cannont compute date, get dates based on  counts reproted in the codebook

nshap_date_codes_r2 <- tibble::tribble(
  ~real_date_r2,   ~count,
  "2010-12", 215L,
  "2010-11", 432L,
  "2010-10", 902L,
...
r2_dates <- main %>% filter(wave_id == "R2") %>% group_by(date) %>% 
  summarise(count = n()) %>% 
  left_join(nshap_date_codes_r2, by = "count") %>% select(-count) %>% 
  mutate(real_date_r2 = as.Date(paste0(real_date_r2, "-15"), format = "%Y-%m-%d"))




nshap_date_codes_r3 <- tibble::tribble(
  ~real_date_r3,   ~count,
  "2015-12",  438L,
  "2015-11",  709L,
  "2015-10", 1071L,
...
r3_dates <- main %>% filter(wave_id == "R3") %>% group_by(date) %>% 
  summarise(count = n()) %>% 
  left_join(nshap_date_codes_r3, by = "count") %>% select(-count) %>% 
  mutate(real_date_r3 = as.Date(paste0(real_date_r3, "-15"), format = "%Y-%m-%d"))

main <- main %>%
  left_join(r2_dates, by = "date") %>%
  left_join(r3_dates, by = "date")

main <- main %>% 
  mutate(date = case_when(wave_id == "R1" ~ as.Date(paste0(gsub("m", "-", date), "-15"), format = "%Y-%m-%d"),
                          wave_id == "R3" ~ real_date_r3,
                          wave_id == "R2" ~ real_date_r2)) %>%
  select(-c(real_date_r3, real_date_r2)) %>% 
  group_by(wave_id) %>% 
...
# only keep respondents aged between 45 & 90y.o.
main <- main %>%  filter(age %in% c(40:90))
...
main <- main %>% mutate(CIGS_DAY_OEC = case_when(SMK_CHECK == 0 ~ 0,
                                                 TRUE ~ CIGS_DAY_OEC),
                        ALC_WK_DIS8A = case_when(wave_id == "R3" & ALC_CHECK3 == 0 ~ 0,
                                                 wave_id %in% c("R1", "R2") & (ALC_CHECK == 0| ALC_CHECK2 == 0| ALC_CHECK3 == 0) ~ 0,
                                                 TRUE ~ ALC_WK_DIS8A),
                        ALC_DRINKS_DAY_OEA = case_when(wave_id == "R3" & ALC_CHECK3 == 0 ~ 0,
                                                       wave_id %in% c("R1", "R2") & (ALC_CHECK == 0| ALC_CHECK2 == 0| ALC_CHECK3 == 0) ~ 0,
                                                       TRUE ~ ALC_DRINKS_DAY_OEA),
                        ALC_4DRINKS_3MO_DIS93A = case_when(wave_id == "R3" & ALC_CHECK3 == 0 ~ 0,
                                                           wave_id %in% c("R1", "R2") & (ALC_CHECK == 0| ALC_CHECK2 == 0| ALC_CHECK3 == 0) ~ 0,
...
vars_to_keep <- main  %>% 
  ...
  filter(!var_name %in% dependency_vars) %>%  # ignore vars to check for dependencie
...
# dim(risk_info_analyse)  9 rows x  13 cols
