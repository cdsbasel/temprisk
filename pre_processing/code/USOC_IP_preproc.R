
##### PRE-PROCESSING: #####

#  DESCRIPTION -------------------------------------------------------------

#This script uses the file containing information on the risk measures & other variables of interest
# to select the relevant columns in the panel's original dataset. Where relevant, recoding/fixing of variables is done. 
# The distribution of risk preference measures is checked + other data quality checks are performed.
# A separate csv file of the risk preference measures to use to calculate retest & inter- correlations is saved. 
# The processed/clean data is saved as a csv file (long format). 

# File input: (need to specify location of files)
# panel_variable_info: rds file containing information of the variables to extract from each wave of the panel 
# PANEL data: files with the original panel data 

# File output:(need to specify location of storage for files) 
# PANEL_proc_data.csv : clean/processed panel data with variables of interest in LONG format
# PANEL_risk_var_info.csv : list of risk preference measures that will be used to calculate retest & inter- correlations
# PANEL_inconsistent_dems.csv : summary table of the % of (unique) respondents with year of birth or gender that is not consistently reported across waves 
# PANEL_resp_overview.html : summary statistics of the data in PANEL_proc_data.csv by wave id

# Author(s): Alexandra Bagaini and Rui Mata, Centre for Cognitive and Decision Sciences, Faculty of Psychology, University of Basel.

# PACKAGES ---------------------------------------------------------------

library(tidyverse) # data wrangling 
library(readxl) # reading excel file
library(lubridate) # dealing with dates
library(summarytools) # data check
library(foreign) # open spss file


# PATH INFORMATION -----------------------------------------------------

panel_data_path <- c("~/Documents/PROJECTS/TSRP/Data/USOC_IP/RawData/") # where the raw panel data is stored
measure_info_path <- c("VariableInfo/") # location of xlsx file containing info on the panel's risk measures
preproc_data_path <- c("~/Documents/PROJECTS/TSRP/Data/USOC_IP/ProcData/") # where to save processed panel data

# VARIABLE INFORMATION -----------------------------------------------------

# PATH INFORMATION -----------------------------------------------------

var_book <- read_rds("var_info/panel_variable_info.rds")# panel risk measures
measure_info_path <- c("var_info/indv_panel_var_info/") 
panel_data_path <- c("") # where the raw panel data is stored
preproc_data_path <- c("") # where to save processed panel data
panel_name <- "USOC_IP"

# VARIABLE INFORMATION ----------------------------------------------------

panel_var <- var_book[[panel_name]]
# combining id & risk information
all_info <- bind_rows(panel_var$Measures, panel_var$ID, panel_var$Dems, panel_var$Dates)
risk_info <- panel_var$Measures
newvars_info <- panel_var$NewVars
# READING DATA: INDALL FILES -----------------------------------------------------

# data to find missing yob or gender information
indall_info <- all_info %>% filter(grepl("indall",varfile))

#________________READ MAIN DATA FILES___________________# 

# list of unique waves
wave_list <- unique(na.omit(indall_info$wave_id)) 
data_list <- NULL
wave_num <- 1

for (CurrWave in wave_list) {
  
  # select wave relevant info
  sub_all_info <- indall_info %>% filter(wave_id == CurrWave)
  
  vars_of_interest <- sub_all_info$origin_varcode # vars of interest
  file_of_interest_name <- unique(na.omit(sub_all_info$varfile)) # in which file are the relevant vars stored
  file_of_interest_name <- paste0(file_of_interest_name,".sav") # spss file
  
  file_of_interest <- list.files(path = panel_data_path, pattern = file_of_interest_name, full.names = TRUE, recursive = TRUE)
  
  # open file
  wave_data <-read.spss(file_of_interest, to.data.frame = TRUE, use.value.labels = FALSE)
  
  # select vars of interest
  wave_data <- wave_data %>% select(all_of(vars_of_interest)) %>%  mutate(wave_id = CurrWave)
  
  # replacing var codes
  colnames_found <- match(colnames(wave_data), vars_of_interest, nomatch = 0)
  colnames(wave_data)[colnames(wave_data) %in% vars_of_interest] <- sub_all_info$varcode[colnames_found]
  
  # place in list
  data_list[[wave_num]] <- wave_data
  wave_num <- wave_num + 1
}

#________________BIND DATA FROM DIFFERENT WAVES TOGETHER___________________# 

dem_indall <- bind_rows(data_list)
dem_indall <- dem_indall %>% mutate(id = as.character(id)) %>% distinct(id, yob ,gender)
# dim(dem_indall)  9348 rows x 3 cols

# FIX VARS: INDALL FILES ONLY ---------------------------------------------
#________________REPLACING MISSING & NON READABLE RESPONSES___________________#
dem_indall <- dem_indall %>% 
  replace(.< 0 , NA_real_ ) # any vars with values below 0 (i.e., invalid, DK, proxy, missing,inapplicable....) are replaced  

# for some years, variables are empty but can be filled/reconstructed from previous or future waves...
dem_indall <- dem_indall %>% 
  group_by(id) %>% 
  fill(yob,.direction = 'downup') %>%  # NAs get replaced with neighboring yob for each respondent 
  ungroup() 

dem_indall <- dem_indall %>% 
  group_by(id) %>% 
  fill(gender,.direction = 'downup') %>%  # NAs get replaced with neighboring yob for each respondent 
  ungroup() 

#avoid id-yob-gender that are not unique
dem_indall <- dem_indall %>% 
  group_by(id) %>% 
  mutate(id_count = n()) %>%
  ungroup() %>% 
  filter(id_count == 1) %>% select(-id_count)

# READING DATA: HRRESP FILES -----------------------------------------------------

# not all files have individual interview dates, use household interview dates as proxy
hhresp_info <- all_info %>% filter(grepl("hhresp",varfile))

#________________READ MAIN DATA FILES___________________# 

# list of unique waves
wave_list <- unique(na.omit(hhresp_info$wave_id)) 
data_list <- NULL
wave_num <- 1

for (CurrWave in wave_list) {
  
  # select wave relevant info
  sub_all_info <- hhresp_info %>% filter(wave_id == CurrWave)
  
  vars_of_interest <- sub_all_info$origin_varcode # vars of interest
  file_of_interest_name <- unique(na.omit(sub_all_info$varfile)) # in which file are the relevant vars stored
  file_of_interest_name <- paste0(file_of_interest_name,".sav") # spss file
  
  file_of_interest <- list.files(path = panel_data_path, pattern = file_of_interest_name, full.names = TRUE, recursive = TRUE)
  
  # open file
  wave_data <-read.spss(file_of_interest, to.data.frame = TRUE, use.value.labels = FALSE)
  
  # select vars of interest
  wave_data <- wave_data %>% select(all_of(vars_of_interest)) %>%  mutate(wave_id = CurrWave)
  
  # replacing var codes
  colnames_found <- match(colnames(wave_data), vars_of_interest, nomatch = 0)
  colnames(wave_data)[colnames(wave_data) %in% vars_of_interest] <- sub_all_info$varcode[colnames_found]
  
  # place in list
  data_list[[wave_num]] <- wave_data
  wave_num <- wave_num + 1
}

#________________BIND DATA FROM DIFFERENT WAVES TOGETHER___________________# 

main_hhresp <- bind_rows(data_list)
main_hhresp <- main_hhresp %>% mutate(hid = as.character(hid))
# dim(main_hhresp)  17381 rows x  4 cols


# READING DATA: INDRESP FILES -----------------------------------------------------

indresp_info <- all_info %>% filter(grepl("indresp",varfile))

#________________READ MAIN DATA FILES___________________# 

# list of unique waves
wave_list <- unique(na.omit(indresp_info$wave_id)) 
data_list <- NULL
wave_num <- 1

for (CurrWave in wave_list) {
  
  # select wave relevant info
  sub_all_info <- indresp_info %>% filter(wave_id == CurrWave)
  
  vars_of_interest <- sub_all_info$origin_varcode # vars of interest
  file_of_interest_name <- unique(na.omit(sub_all_info$varfile)) # in which file are the relevant vars stored
  file_of_interest_name <- paste0(file_of_interest_name,".sav") # spss file
  
  file_of_interest <- list.files(path = panel_data_path, pattern = file_of_interest_name, full.names = TRUE, recursive = TRUE)
  
  # open file
  wave_data <-read.spss(file_of_interest, to.data.frame = TRUE, use.value.labels = FALSE)
  
  # select vars of interest
  wave_data <- wave_data %>% select(all_of(vars_of_interest)) %>%  mutate(wave_id = CurrWave)
  
  # replacing var codes
  colnames_found <- match(colnames(wave_data), vars_of_interest, nomatch = 0)
  colnames(wave_data)[colnames(wave_data) %in% vars_of_interest] <- sub_all_info$varcode[colnames_found]
  
  # place in list
  data_list[[wave_num]] <- wave_data
  wave_num <- wave_num + 1
}

#________________BIND DATA FROM DIFFERENT WAVES TOGETHER___________________# 

main_indresp <- bind_rows(data_list)
main_indresp <- main_indresp %>% mutate(id = as.character(id), hid = as.character(hid))
main_indresp <- main_indresp[ , order(names(main_indresp))]
# dim(main_indresp)  29964 rows x  38 cols

# FIX VARS: INDRESP FILES ONLY --------------------------------------------------------------------

#________________REPLACING MISSING & NON READABLE RESPONSES___________________#
main_indresp <- main_indresp %>% 
  replace(.< 0 , NA_real_ ) # any vars with values below 0 (i.e., invalid, DK, proxy, missing,inapplicable....) are replaced  


#________________FIX VARS: RISK PREF. MEASURES___________________#

# CHOICE_HL_HISTA_* composite measure  
# Risky choice = 1, safe choices = 0
subtract <- function(x) (x - 1)
main_indresp <- main_indresp %>% mutate_at(vars(CHOICE_HL_HISTA_1:CHOICE_HL_HISTA_9), subtract)
main_indresp <- main_indresp  %>% rowwise() %>% 
  mutate(HS_LOTTERY_RISK = (1/9)*sum(c_across(CHOICE_HL_HISTA_1:CHOICE_HL_HISTA_9), na.rm = FALSE)) %>% ungroup()

# CHOICE_HL_LOWSTA_* composite measure  
# Risky choice = 1, safe choices = 0
main_indresp <- main_indresp %>% mutate_at(vars(CHOICE_HL_LOWSTA_1:CHOICE_HL_LOWSTA_9), subtract)
main_indresp <- main_indresp  %>% rowwise() %>% 
  mutate(LS_LOTTERY_RISK = (1/9)*sum(c_across(CHOICE_HL_LOWSTA_1:CHOICE_HL_LOWSTA_9), na.rm = FALSE)) %>% ungroup()


# SMOKING
# Dependencies (refer to codebook)
main_indresp <- main_indresp %>%  
  
  # some waves asked if ever smoked (if not, this should be indicated as 2 for not currently smoking) 
  mutate(SMK_NOW_CHECK = case_when(SMK_EVER_CHECK == 2 & wave_id ==  "W5" ~ 2,
                                   TRUE ~ SMK_NOW_CHECK), 
         
         # harmonized smoker check variable
         SMK_CHECK_H = case_when(wave_id ==  "W5" ~ SMK_NOW_CHECK,
                                 wave_id %in% c("W7", "W8", "W9", "W10", "W11", "W13") ~ SMK_CHECK,
                                 TRUE ~ NA_real_), 
         
         CIGS_DAY_OED = case_when(SMK_CHECK_H == 2 ~ 0, # non smokers do not get asked question --> get 0 
                                  SMK_CHECK_H == 1 & CIGS_DAY_OED == 0 ~ 1, # 0 still means smoking but less than one a day
                                  is.na(SMK_CHECK_H) ~ NA_real_,
                                  TRUE ~ CIGS_DAY_OED + 1))


# ALCOHOL
# Dependencies (refer to codebook)
main_indresp <- main_indresp %>%  
  
  # for those that never drink they should get never or none instead of NA for some questions
  mutate(ALC_MO_ORD5B = case_when(wave_id %in% c("W6", "W7", "W4", "W12", "W13") & ALC_EVER_CHECK == 2 ~ 5,
                                  TRUE ~ ALC_MO_ORD5B),
         
         ALC_5DRINKS_MO_ORD6A = case_when(wave_id %in% c("W6", "W7", "W4", "W12", "W13")  & c(ALC_EVER_CHECK == 2 | ALC_MO_ORD5B == 5) ~ 1,
                                          TRUE ~ ALC_5DRINKS_MO_ORD6A),
         
         ALC_DRUNK_MO_ORD7A = case_when(wave_id %in% c("W4", "W12", "W13")  & c(ALC_EVER_CHECK == 2 | ALC_MO_ORD5B == 5) ~ 1,
                                        TRUE ~ ALC_DRUNK_MO_ORD7A))


# reverse scoring
main_indresp <- main_indresp %>%
  mutate(ALC_MO_ORD5B = 6 - ALC_MO_ORD5B)


# PROPENSITY RISK
# Reverse coding (not indicated in Galizzi et al. (2016) or questionnaires, but when checking the data these seem to be reversed in W7)
main_indresp <- main_indresp %>% mutate(RISK_FIN_ORD11A = case_when(wave_id == "W7" ~ 10 - RISK_FIN_ORD11A ,
                                                                    TRUE ~ RISK_FIN_ORD11A),
                                        RISK_HEA_ORD11A = case_when(wave_id == "W7" ~ 10 - RISK_HEA_ORD11A ,
                                                                    TRUE ~ RISK_HEA_ORD11A),
                                        RISK_GEN_ORD11A = case_when(wave_id == "W7" ~ 10 - RISK_GEN_ORD11A ,
                                                                    TRUE ~ RISK_GEN_ORD11A))




# READING DATA: YOUTH FILES -----------------------------------------------------

youth_info <- all_info %>% filter(grepl("youth",varfile))

#________________READ MAIN DATA FILES___________________# 

# list of unique waves
wave_list <- unique(na.omit(youth_info$wave_id)) 
data_list <- NULL
wave_num <- 1

for (CurrWave in wave_list) {
  
  # select wave relevant info
  sub_all_info <- youth_info %>% filter(wave_id == CurrWave)
  
  vars_of_interest <- sub_all_info$origin_varcode # vars of interest
  file_of_interest_name <- unique(na.omit(sub_all_info$varfile)) # in which file are the relevant vars stored
  file_of_interest_name <- paste0(file_of_interest_name,".sav") # spss file
  
  file_of_interest <- list.files(path = panel_data_path, pattern = file_of_interest_name, full.names = TRUE, recursive = TRUE)
  
  # open file
  wave_data <-read.spss(file_of_interest, to.data.frame = TRUE, use.value.labels = FALSE)
  
  # select vars of interest
  wave_data <- wave_data %>% select(all_of(vars_of_interest)) %>%  mutate(wave_id = CurrWave)
  
  # replacing var codes
  colnames_found <- match(colnames(wave_data), vars_of_interest, nomatch = 0)
  colnames(wave_data)[colnames(wave_data) %in% vars_of_interest] <- sub_all_info$varcode[colnames_found]
  
  # place in list
  data_list[[wave_num]] <- wave_data
  wave_num <- wave_num + 1
}

#________________BIND DATA FROM DIFFERENT WAVES TOGETHER___________________# 

main_youth <- bind_rows(data_list)
main_youth <- main_youth %>% mutate(id = as.character(id), hid = as.character(hid))
# dim(main_youth)  2157 rows x  12 cols

# FIX VARS: YOUTH FILES ONLY --------------------------------------------------------------------

#________________REPLACING MISSING & NON READABLE RESPONSES___________________#
main_youth <- main_youth %>% 
  replace(.< 0 , NA_real_ ) # any vars with values below 0 (i.e., invalid, DK, proxy, missing,inapplicable....) are replaced  

#________________FIX VARS: RISK PREF. MEASURES___________________#

# ALCOHOL
# Dependencies (refer to codebook)
main_youth <- main_youth %>%  
  
  # for those that never drink they should get never or none instead of NA for some questions
  mutate(ALC_MO_ORD5B = case_when(wave_id %in% c("W4", "W6", "W8", "W10", "W12", "W2") & ALC_EVER_CHECK == 2 ~ 5,
                                  # For sone wave, when ALC_MO_ORD5B == 6, that is when the respondent selected "5 (Never)" and still indicated whether
                                  # or not they drank alcohol recently (we fix this)
                                  wave_id %in% c("W3", "W5", "W7", "W9", "W11","W13") & ALC_MO_ORD5B == 6 & ALC_5DRINKS_MO_ORD6A == 1 ~ 5,
                                  wave_id %in% c("W3", "W5", "W7", "W9", "W11", "W13") & ALC_MO_ORD5B == 6 & ALC_5DRINKS_MO_ORD6A != 1 ~ NA_real_,
                                  TRUE ~ ALC_MO_ORD5B),
         ALC_5DRINKS_MO_ORD6A = case_when(wave_id %in% c("W3", "W5", "W7", "W9", "W11", "W13") & ALC_MO_ORD5B == 5 ~ 1,
                                          TRUE ~ ALC_5DRINKS_MO_ORD6A))


# reverse scoring
main_youth <- main_youth %>%  
  mutate(ALC_MO_ORD5B = 6 - ALC_MO_ORD5B)

# CREATE MAIN DF WITH YOUTH, INDRESP AND HHRESP FILES  -----------------------------------------------------------------

main <- bind_rows(main_indresp, main_youth)

main <- main %>%  left_join(main_hhresp, by = c("hid", "wave_id")) %>% select(-hid)



# FIX VARS: COMBINED FILE -------------------------------------------------


#________________FIX VARS: REMOVE DUPLICATE IDS ___________________#

#avoid IDs that within a wave are not unique
main <- main %>% 
  group_by(id, wave_id) %>% 
  mutate(id_count = n()) %>%
  ungroup() 

main %>% group_by(wave_id) %>% summarise(sum(id_count > 1)) 
main <- main %>%  filter(id_count == 1) %>% select(-id_count)

#________________FIX VARS: DATES ___________________#
# If month & date are missing replace with the mean of available dates.
# If date information is completely missing from the raw data AND cannot be estimated from panel documentation, use 15th of June 

main <- main %>% mutate(date = as.Date(paste(YINT,MINT,"15",sep="-"), "%Y-%m-%d"))

# some rows with missing dates (replace with mean)
main <- main %>% group_by(wave_id) %>% mutate(date = case_when(is.na(date) ~ mean(date, na.rm = T),
                                                               TRUE ~ date)) %>% ungroup() %>% select(-c(MINT, YINT))

main <- main %>% group_by(wave_id) %>% mutate(wave_year = round(mean(year(date)))) %>% ungroup() # calculate the general year for that wave


#________________FIX YOB & AGE & AGE RANGE___________________#

# for some years, variables are empty but can be filled/reconstructed from previous or future waves...
main <- main %>% 
  group_by(id) %>% 
  fill(yob,.direction = 'downup') %>%  # NAs get replaced with neighboring yob for each respondent 
  ungroup() 

# calculate age (if missing)
main <- main %>% 
  mutate(age = case_when(is.na(age) ~ year(date) - yob,
                         TRUE ~ age))

# Some respondents w/o age or yob information in wave data files, use data from indall to complete
missing_age_yob_tracker <- dem_indall %>% filter(id %in% unique(main$id[is.na(main$age)])) %>% select(id, yob) %>% rename(yob2 = yob)
main <- main %>% left_join(missing_age_yob_tracker, by = "id") %>% mutate(yob = case_when(is.na(yob) & !is.na(yob2) ~ yob2,
                                                                                          TRUE ~ yob)) %>% select(-yob2)


# calculate age again (if missing)
main <- main %>% 
  mutate(age = case_when(is.na(age) ~ year(date) - yob,
                         TRUE ~ age))


# identify respondents with not the same yob reported across the waves
main <- main %>%
  group_by(id) %>%
  mutate(non_unique_yob = length(unique(na.omit(yob))) > 1) %>% #
  ungroup() %>% select(-yob)


# Some respondents with non-unique yobs, could use tracker data to define a unique yob however it is possible that different people
#  completed the interview in different waves. For data quality, we could alternatively exclude entries where the yob is not consistent
# with the tracker file. However, for consistency with other panels we will exclude these respondents completely. 

# only keep respondents aged between 10 & 90y.o.
main <- main %>%  filter(age %in% c(10:90)) 

#________________FIX VARS: GENDER___________________#

# for some years, variables are empty but can be filled/reconstructed from previous or future waves...
main <- main %>% 
  group_by(id) %>% 
  fill(gender,.direction = 'downup') %>%  # NAs get replaced with neighboring yob for each respondent 
  ungroup() 



#  identify respondents with not the same gender reported across the waves
main <- main %>%
  group_by(id) %>%
  mutate(non_unique_gender = length(unique(na.omit(gender))) > 1) %>%  
  ungroup()


# recode gender 
main <- main %>%
  mutate(gender = case_when(gender == 2 ~ 1,
                            gender == 1 ~ 0)) # male = 0, female = 1


# A few respondents with not the same gender reported across the waves, could use tracker data to define whether a respondent is male or female,
#  however it is possible that different people completed the interview in different waves. For data quality, we could alternatively exclude entries
#   where the gender is not consistent with the tracker file. However, for consistency with other panels we will exclude these respondents completely. 

inconsistent_dems <- main %>%
  distinct(id, non_unique_gender, non_unique_yob) %>%
  summarise(perc_gender = 100*(sum(non_unique_gender)/n()),
            perc_yob = 100*(sum(non_unique_yob)/n())) %>%
  mutate(panel = panel_name)


# CHECK VARS: COMBINED FILE --------------------------------------------------------------

#________________ CHECK VARS: FILTER OUT RESPONDENTS WITH MISSING DEMS___________________#

main <- main %>% 
  filter(!is.na(age)) %>% #keep rows that have age info
  filter(!is.na(gender)) %>%  #keep rows that have gender info
  filter(non_unique_gender == 0) %>% # filter out respondents with not the same gender reported across the waves 
  filter(non_unique_yob == 0)  # filter out respondents with not the same yob reported across the waves 

#________________ CHECK VARS: DATA VISUAL (I)___________________#

# distribution of responses:
# summarytools::view(by(main, main$wave_id, dfSummary))

#________________ CHECK VARS: MEASURE QUAL. CRITERIA ___________________#
# exclude measures if:
# - in none of waves there is at least 4 different values possible for a measure (i.e., limited range of responses)

dependency_vars <- c(colnames(main)[grepl("*CHECK", colnames(main))], # vars to check for dependencies
                     colnames(main)[grepl("CHOICE", colnames(main))]) # vars of yes/no choices in lottery
vars_to_keep <- main  %>% 
  pivot_longer(!c(id, wave_id, wave_year, date, gender, age, non_unique_gender, non_unique_yob), values_to = "response", names_to = "var_name") %>% 
  filter(!is.na(response)) %>% 
  filter(!var_name %in% dependency_vars) %>%  # ignore vars to check for dependencies
  group_by(var_name, wave_id) %>%
  # how many unique responses possible per measure 
  mutate(opt_per_var = length(unique(na.omit(response)))) %>%  
  ungroup() %>% 
  distinct(wave_id, var_name, opt_per_var) %>% 
  group_by(var_name) %>% 
  # In at least ONE wave is there at least 4 different values possible per measure?
  mutate(var_include = if_else(sum(opt_per_var >= 4) >= 1, 1, 0)) %>% 
  ungroup()

# list of vars to keep
vars_to_keep <- vars_to_keep  %>% 
  filter(var_include == 1) %>% distinct(var_name)

# create a descriptive overview of vars to analyse
risk_info_analyse <- risk_info %>% bind_rows(newvars_info) %>% distinct(panel, varcode, measure_category, general_domain, domain_name,
                                                                        scale_type,scale_length, time_frame, behav_type, behav_paid) %>% 
  mutate(var_consider = if_else(varcode %in% dependency_vars, 0, 1), # considering non-dependency vars (or binary vars) for calculating retest correlations
         var_include = if_else(varcode %in% vars_to_keep$var_name, 1,0)) # include or exclude from calculating retest correlations
# dim(risk_info_analyse)  36 rows x  12 cols

# CREATE CLEAN/PROCESSED DATAFRAME  --------------------------------------------------------------

#________________SELECT VARS OF INTEREST FOR RETEST CALC___________________#

proc_data <- main %>% 
  # keep necessary vars (i.e., vars_to_keep and exclude vars used to check for dependencies)
  select(id, age, gender, wave_id,wave_year,date, # dems
         all_of(vars_to_keep$var_name)) %>% #risk measures
  filter_at(all_of(vars_to_keep$var_name),any_vars(!is.na(.))) #keep rows that have at least one risk value
# dim(proc_data)  21583 rows x  20 cols

# long format
proc_data_l <- pivot_longer(proc_data,all_of(vars_to_keep$var_name),names_to = "varcode", values_to = "response")
# dim(proc_data_l)  302162 rows x  8 cols

# checking if all id-gender combinations are unique
# proc_data_l %>%  distinct(id, gender) %>% summarise(check = n() == length(unique(proc_data$id)))
# checking if all id-varcode-wave combinations are unique
# t <- proc_data_l %>%  group_by(id, wave_id, varcode) %>% summarise(check = n(), .groups = "drop"); sum(t$check > 1) == 0


#________________ CHECK VARS: DATA VISUAL (II)___________________#

# distribution of responses:
# summarytools::view(by(proc_data, proc_data$wave_id, dfSummary))



# ADD PANEL INFORMATION ---------------------------------------------------


proc_data_l <- proc_data_l %>% 
  bind_cols(panel_var$PanelInfo) %>% 
  relocate(panel, .before = id)



# SAVING OUTPUT --------------------------------------------------------------

# saving clean/processed data "long"
write_csv(proc_data_l,paste0(preproc_data_path, panel_name, "_proc_data.csv")) 

# saving description of vars to analyse
write_csv(risk_info_analyse,paste0(measure_info_path, panel_name, "_risk_var_info.csv")) 

# ids with inconsistent dems count
write_csv(inconsistent_dems,paste0(preproc_data_path, panel_name, "_inconsistent_dems.csv")) 

# distribution of responses:
summarytools::view(by(proc_data, proc_data$wave_id, dfSummary), file = paste0(preproc_data_path, panel_name, "_resp_overview.html"))

print(paste0(panel_name, " pre-processing done!"))

