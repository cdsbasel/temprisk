...
panel_data_path <- c("~/Documents/TSRP/Data/MIDJA/RawData/") # where the raw panel data is stored
preproc_data_path <- c("~/Documents/TSRP/Data/MIDJA/ProcData/") # where to save processed panel data
panel_name <- "MIDJA"
...
main <- main %>%  mutate(ALC_5DRINKS_MO_OEA = case_when(is.na(ALC_5DRINKS_MO_OEA)& ALC_MO_ORD6A == 6  ~ 0, # if people report not having ever tried alcohol they get never instead of NA
                                                        TRUE ~ ALC_5DRINKS_MO_OEA),
                         ALC_OVERCONS_YR_ORD6A = case_when(is.na(ALC_OVERCONS_YR_ORD6A)& ALC_MO_ORD6A == 6  ~ 1, # if people report not having ever tried alcohol they get never instead of NA
                                                        TRUE ~ ALC_OVERCONS_YR_ORD6A),
                         ALC_EFFECT_YR_ORD6A = case_when(is.na(ALC_EFFECT_YR_ORD6A)& ALC_MO_ORD6A == 6 ~ 1, # if people report not having ever tried alcohol they get never instead of NA
                                                           TRUE ~ ALC_EFFECT_YR_ORD6A),
                         ALC_DRINKS_DAY_OEB = case_when(is.na(ALC_DRINKS_DAY_OEB)& ALC_MO_ORD6A == 6 ~ 0, # if people report not having ever tried alcohol they get never instead of NA
...
main <- main %>% mutate(date = case_when(wave_id == "W2" ~ as.Date(paste(2012,7,31,sep="-"), "%Y-%m-%d"),
                                         wave_id == "W1" ~  as.Date(paste(2008,7,31,sep="-"), "%Y-%m-%d"))) 
...
# dim(risk_info_analyse)  6 rows x  13 cols
