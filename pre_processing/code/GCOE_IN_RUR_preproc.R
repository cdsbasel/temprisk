...
main <- main %>% mutate(date = case_when(wave_id == "2012" ~ as.Date(paste(as.character(YINT), as.character(MINT),as.character(DINT)), "%Y%m%d"),
...
# calculate age 
main <- main %>% 
  mutate( yob = case_when(yob == 9999 ~ NA_real_, # invalid NAs
                          TRUE ~ yob),
          age = year(date) - yob) 
