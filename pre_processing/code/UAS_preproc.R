...
panel_data_path <- c("~/Documents/TSRP/Data/UAS/RawData/") # where the raw panel data is stored
preproc_data_path <- c("~/Documents/TSRP/Data/UAS/ProcData/") # where to save processed panel data
panel_name <- "UAS"
...
for (CurrWave in wave_list) {
  ...
  # file_of_interest_name <- paste0(file_of_interest_name, ".dta") 
  file_of_interest <- list.files(path = panel_data_path, pattern = file_of_interest_name, full.names = TRUE, recursive = TRUE) 
  
  # open the relevant file
  wave_data <- read_dta(file_of_interest, encoding='latin1') # opens the relevant file (dta format)
  
  
  
  # select vars of interest
  vars_of_interest <- tolower(sub_all_info$origin_varcode)
  wave_data <- wave_data %>% select(all_of(vars_of_interest)) %>%  mutate(wave_id = CurrWave)
  
  # replacing var codes
  if (CurrWave == "UAS117"){
    
    sub_all_info <-  sub_all_info %>%
      mutate(varcode = case_when(origin_varcode == "exp_2B" ~ paste0(varcode, "_b"),
                                 TRUE ~ varcode))
  }
  ...
  # place in list
  data_list[[wave_num]] <- wave_data %>% mutate(yob = as.numeric(yob))
  wave_num <- wave_num + 1
...
# dim(main) 242405 rows  x  25 cols
# FIX VARS  --------------------------------------------------------------------


main2 <- main %>% filter(wave_id == "UAS244") %>% select(id, ALC_4DRINKS_WK_DIS8Am, ALC_4DRINKS_WK_DIS8Af, ALC_CHECK, ALC_WK_DIS8B)
...
main <- main %>% mutate(ALC_DRINKS_DAY_OED = case_when(ALC_WK_DIS8B == 0 & !wave_id %in% c("UAS230", "UAS235") ~ 0,
                                                       TRUE ~ ALC_DRINKS_DAY_OED),
                        ALC_WK_DIS8A = case_when(ALC_CHECK != 1 ~ 0,
                                                 ALC_WK_DIS8A > 7 ~ NA,  # out of range values removed
                                                 TRUE ~ ALC_WK_DIS8A),
                        ALC_DRINKS_DAY_OEA = case_when(ALC_CHECK != 1 ~ 0,
                                                       TRUE ~ ALC_DRINKS_DAY_OEA),
                        ALC_4DRINKS_3MO_DIS93A = case_when(ALC_CHECK != 1 ~ 0,
                                                           ALC_4DRINKS_3MO_DIS93A > 92 ~ NA,   # out of range values removed
                                                           TRUE ~ ALC_4DRINKS_3MO_DIS93A), 
                        ALC_4DRINKS_WK_DIS8Af = case_when(ALC_WK_DIS8B == 0 & gender == 1  ~ 0,
                                                          ALC_4DRINKS_WK_DIS8Af > 7 ~ NA,  # out of range values removed
                                                          TRUE ~ ALC_4DRINKS_WK_DIS8Af),
                        ALC_4DRINKS_WK_DIS8Am = case_when(ALC_WK_DIS8B == 0 & gender == 0 ~ 0,
                                                          ALC_4DRINKS_WK_DIS8Am > 7 ~ NA, 
                                                          TRUE ~ ALC_4DRINKS_WK_DIS8Am)) %>% 
  mutate(ALC_4DRINKS_WK_DIS8A = coalesce(ALC_4DRINKS_WK_DIS8Af, ALC_4DRINKS_WK_DIS8Am)) %>%
  select(-c(ALC_4DRINKS_WK_DIS8Af, ALC_4DRINKS_WK_DIS8Am))

# SMOKING
# Dependencies (refer to codebook)                        
main <- main %>% mutate(CIGS_DAY_OEE = case_when(SMKEVER_CHECK == 5 | SMKNOW_CHECK == 5 ~ 0,
                                                 TRUE ~ CIGS_DAY_OEE),
                        PACKS_DAY_OEE = case_when(SMKEVER_CHECK == 5 | SMKNOW_CHECK == 5 ~ 0,
                                                  TRUE ~ PACKS_DAY_OEE)) %>% 
  # convert pack units to cigarette units
  mutate(CIGS_DAY_OEE = case_when(is.na(CIGS_DAY_OEE) & !is.na(PACKS_DAY_OEE) ~ PACKS_DAY_OEE * 20,
                                  TRUE ~ CIGS_DAY_OEE)) %>% select(-PACKS_DAY_OEE)


# PROPENSITY
# Harmonize scales (from 0 to 10)
main <- main %>% mutate(RISK_FIN_ORD11A = case_when(wave_id == "UAS166" ~ RISK_FIN_ORD11A - 1,
                                                    RISK_FIN_ORD11A == 98 ~ NA_real_,
...
# BEHAVIOUR ---------------------------------------------------------------
# lottery reverse
# account for question order in survey 117
main117 <- main %>% filter(wave_id %in% c("UAS117")) %>% 
  mutate(LOTTERY_ALLOCA = coalesce(LOTTERY_ALLOCA, LOTTERY_ALLOCA_b))

main <- main %>% 
  filter(wave_id != "UAS117") %>% 
  bind_rows(main117) %>% 
  mutate(LOTTERY_ALLOCA =  (10000 - LOTTERY_ALLOCA)) %>% 
  select(-LOTTERY_ALLOCA_b)
...
# dim(risk_info_analyse)  19 rows x 13 cols
