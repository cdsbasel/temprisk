...
panel_data_path <- c("/Users/abagaini/Documents/TSRP/Data/FICR/RawData/") # where the raw panel data is stored
preproc_data_path <- c("/Users/abagaini/Documents/TSRP/Data/FICR/ProcData/") # where to save processed panel data
panel_name <- "FICR"
...
main <- main %>% mutate(date = case_when(wave_id == "W3" ~  as.Date("2008-11-06"),
                                         wave_id == "W4" ~  as.Date("2008-12-12"),
                                         wave_id == "W5" ~  as.Date("2009-03-24"),
                                         wave_id == "W6" ~ as.Date("2009-06-04"),
                                         wave_id == "W7" ~ as.Date("2009-10-06")))
...
risk_info_analyse <- risk_info  %>% distinct(panel, varcode, measure_category, general_domain, domain_name,
  ...
  mutate( var_consider = 1, # considering non-dependency vars (or binary vars) for calculating retest correlations
