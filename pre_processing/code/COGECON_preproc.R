...
library(readstata13) # open dta file
...
panel_data_path <- c("~/Documents/TSRP/Data/COGECON/RawData/") # where the raw panel data is stored
preproc_data_path <- c("~/Documents/TSRP/Data/COGECON/ProcData/") # where to save processed panel data
panel_name <- "COGECON"
...
for (CurrWave in wave_list) {
  ...
  wave_data <-read.dta13(file = file_of_interest, convert.factors = FALSE)
...
main <- main %>%  mutate(date = as.Date(paste(as.character(YINT),as.character(MINT),"15",sep="-"), "%Y-%m-%d")) %>% select(-c(YINT,MINT))
...
# "Individuals born in 1956 or earlier (i.e., age 51+) were eligible for the study, as were their
# significant others, regardless of age."
...
# inconsistent yes/no coding across waves
main <- main %>%  mutate(CHOICE_INHERIT_33CUT = case_when(wave_id %in% c("2011", "2013") & CHOICE_INHERIT_33CUT == 0 ~ 5,
                                                          TRUE ~ CHOICE_INHERIT_33CUT),
                         CHOICE_INHERIT_20CUT = case_when(wave_id %in% c("2011", "2013") & CHOICE_INHERIT_20CUT == 0 ~ 5,
                                                          TRUE ~ CHOICE_INHERIT_20CUT),
                         CHOICE_INHERIT_10CUT = case_when(wave_id %in% c("2011", "2013") & CHOICE_INHERIT_10CUT == 0 ~ 5,
                                                          TRUE ~ CHOICE_INHERIT_10CUT),
                         CHOICE_INHERIT_50CUT = case_when(wave_id %in% c("2011", "2013") & CHOICE_INHERIT_50CUT == 0 ~ 5,
                                                          TRUE ~ CHOICE_INHERIT_50CUT),
                         CHOICE_INHERIT_75CUT = case_when(wave_id %in% c("2011", "2013") & CHOICE_INHERIT_75CUT == 0 ~ 5,
                                                          TRUE ~ CHOICE_INHERIT_75CUT))
...
# dim(risk_info_analyse)  8 rows x  13 cols
