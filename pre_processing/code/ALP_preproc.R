...
panel_data_path <- c("~/Documents/TSRP/Data/ALP/RawData/")  # where the raw panel data is stored
preproc_data_path <- c("~/Documents/TSRP/Data/ALP/ProcData/")  # where to save processed panel data
panel_name <- "ALP"
...
for (CurrWave in wave_list) {
  ...
  file_of_interest <- unique(na.omit(sub_all_info$varfile)) # in which file are the relevant vars stored
  
  # open file
 wave_data <- read_dta(paste0(panel_data_path,file_of_interest, ".dta")) # less problematic to work with dta files
...
# could potentially use "old_id" to link responses of respondents who moved, but very few of these exist/are readable in the current dataset
# Additionally it is not a variable that is present in all surveys.
# table(main$old_id)

main <- main %>% select(-old_id)
...
main <- main %>% mutate(date = case_when(wave_id == "MS2" ~ as.Date(INT2,"%Y%m%d"),
                                         wave_id != "MS2" ~ as.Date(paste(day(INT),month(INT),year(INT), sep = "-"),"%d-%m-%Y")))
...
# calculate age
main <- main %>% 
  mutate(age = year(date) - (yob + 1900)) %>% select(-yob)
...
# change coding of gender in MS2 (from 2 = Male --> Female & from  1 = Female --> Male )
main <- main %>% mutate(gender = case_when(wave_id == "MS2" ~ 3 - gender,
                                           TRUE ~ gender))
...
# There are NAs for gender information.
...
main <- main %>%  mutate(INCOME_CUT_RISK = case_when(
  ...
  # accepts job with 50% chance of salary 75% cut
...
main <- main %>%  mutate(INHERIT_CUT_RISK = case_when(
  ...
  c(CHOICE_INHERIT_33CUT == 1 | CHOICE_INHERIT_33CUT_DK == 1) & 
    c(is.na(CHOICE_INHERIT_20CUT) | is.na(CHOICE_INHERIT_20CUT_DK)) & 
    c(is.na(CHOICE_INHERIT_10CUT) | is.na(CHOICE_INHERIT_10CUT_DK)) & 
    c(CHOICE_INHERIT_50CUT == 1 | CHOICE_INHERIT_50CUT_DK == 1) & 
    c(CHOICE_INHERIT_75CUT == 1 | CHOICE_INHERIT_75CUT_DK == 1) ~ 6,
  
  # accepts at most investment with 50% chance of 50% cut in inheritance
  c(CHOICE_INHERIT_33CUT == 1 | CHOICE_INHERIT_33CUT_DK == 1) & 
    c(is.na(CHOICE_INHERIT_20CUT) | is.na(CHOICE_INHERIT_20CUT_DK)) & 
    c(is.na(CHOICE_INHERIT_10CUT) | is.na(CHOICE_INHERIT_10CUT_DK)) & 
    c(CHOICE_INHERIT_50CUT == 1 | CHOICE_INHERIT_50CUT_DK == 1) & 
    c(CHOICE_INHERIT_75CUT == 2 | CHOICE_INHERIT_75CUT_DK == 2) ~ 5,
  
  # accepts at most investment with 50% chance of 33% cut in inheritance
  c(CHOICE_INHERIT_33CUT == 1 | CHOICE_INHERIT_33CUT_DK == 1) & 
    c(is.na(CHOICE_INHERIT_20CUT) | is.na(CHOICE_INHERIT_20CUT_DK)) & 
    c(is.na(CHOICE_INHERIT_10CUT) | is.na(CHOICE_INHERIT_10CUT_DK)) & 
    c(CHOICE_INHERIT_50CUT == 2 | CHOICE_INHERIT_50CUT_DK == 2) & 
    c(is.na(CHOICE_INHERIT_75CUT) | is.na(CHOICE_INHERIT_75CUT_DK)) ~ 4, 
  
  # accepts at most investment with 50% chance of 20% cut in inheritance
  c(CHOICE_INHERIT_33CUT == 2 | CHOICE_INHERIT_33CUT_DK == 2) & 
    c(CHOICE_INHERIT_20CUT == 1 | CHOICE_INHERIT_20CUT_DK == 1) & 
    c(is.na(CHOICE_INHERIT_10CUT) | is.na(CHOICE_INHERIT_10CUT_DK)) & 
    c(is.na(CHOICE_INHERIT_50CUT) | is.na(CHOICE_INHERIT_50CUT_DK)) & 
    c(is.na(CHOICE_INHERIT_75CUT) | is.na(CHOICE_INHERIT_75CUT_DK)) ~ 3,
  
  # accepts at most investment with 50% chance of 10% cut in inheritance
  c(CHOICE_INHERIT_33CUT == 2 | CHOICE_INHERIT_33CUT_DK == 2) & 
    c(CHOICE_INHERIT_20CUT == 2 | CHOICE_INHERIT_20CUT_DK == 2) & 
    c(CHOICE_INHERIT_10CUT == 1 | CHOICE_INHERIT_10CUT_DK == 1) & 
    c(is.na(CHOICE_INHERIT_50CUT) | is.na(CHOICE_INHERIT_50CUT_DK)) & 
    c(is.na(CHOICE_INHERIT_75CUT) | is.na(CHOICE_INHERIT_75CUT_DK)) ~ 2,
  
  # accepts none of the risky investments
  c(CHOICE_INHERIT_33CUT == 2 | CHOICE_INHERIT_33CUT_DK == 2) & 
    c(CHOICE_INHERIT_20CUT == 2 | CHOICE_INHERIT_20CUT_DK == 2) & 
    c(CHOICE_INHERIT_10CUT == 2 | CHOICE_INHERIT_10CUT_DK == 2) & 
    c(is.na(CHOICE_INHERIT_50CUT) | is.na(CHOICE_INHERIT_50CUT_DK)) & 
    c(is.na(CHOICE_INHERIT_75CUT) | is.na(CHOICE_INHERIT_75CUT_DK)) ~ 1))



# CHOICE_DRA_1* vars.
# Compute proportion of RISKY choices (i.e., choosing Option A (1) instead of Option B (2))
reverse_scores <- function(x) {abs(2-x)}
main <- main %>%
  # recode such that risky choices = 1 and safe choices = 0
  mutate_at(vars(CHOICE_DRA_1A:CHOICE_DRA_1J),reverse_scores)%>% 
  rowwise() %>% 
  mutate(DRA1_RISK = (1/10)*sum(c_across(CHOICE_DRA_1A:CHOICE_DRA_1J), na.rm = FALSE)) %>%  # needs to be properly ordered
  ungroup()
...
main <- main %>%
  # recode such that risky choices == 1 and safe choices = 0
  mutate_at(vars(CHOICE_DRA_2A:CHOICE_DRA_2J),reverse_scores)%>% 
  rowwise() %>% 
  mutate(DRA2_RISK = (1/10)*sum(c_across(CHOICE_DRA_2A:CHOICE_DRA_2J), na.rm = FALSE)) %>%  # needs to be properly ordered
  ungroup()


# RISK_FIN_ORD10A
# Remove non-valid responses
main <- main %>% mutate(RISK_FIN_ORD10A = if_else(RISK_FIN_ORD10A > 10, NA_real_, RISK_FIN_ORD10A)) 


# recode to march scales form other survey
main <- main %>% mutate(RISK_GEN_ORD11A = if_else(wave_id == "MS48", RISK_GEN_ORD11A - 1, RISK_GEN_ORD11A)) 
...
dependency_vars <- unique(na.omit(risk_info$varcode[risk_info$scale_length == 2])) # vars to check for dependencies/choices
...
# dim(risk_info_analyse)  46 rows x 13 cols
