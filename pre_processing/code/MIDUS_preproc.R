...
panel_data_path <- c("~/Documents/TSRP/Data/MIDUS/RawData/") # where the raw panel data is stored
preproc_data_path <- c("~/Documents/TSRP/Data/MIDUS/ProcData/") # where to save processed panel data
panel_name <- "MIDUS"
...
for (CurrWave in wave_list) {
  ...
  wave_data <-  read_sav(paste0(panel_data_path, file_of_interest))
...
main <- main %>%  mutate(ALC_MO_ORD6A = case_when(is.na(ALC_MO_ORD6A)& wave_id %in% c("W2", "W3") & 
                                                    (ALC_CHECK1 == 96 | ALC_CHECK2 == 2) ~ 6, # if people report not having ever tried alcohol they get never instead of NA
                                                  TRUE ~ ALC_MO_ORD6A),
                         ALC_5DRINKS_MO_OEA = case_when(is.na(ALC_5DRINKS_MO_OEA)& wave_id %in% c("W2", "W3") & 
                                                          (ALC_CHECK1 == 96 | ALC_CHECK2 == 2 | ALC_CHECK3 == 4 |ALC_MO_ORD6A == 6) ~ 0, # if people report not having ever tried alcohol they get never instead of NA
                                                        TRUE ~ ALC_5DRINKS_MO_OEA),
                         ALC_DRINKS_DAY_OEB = case_when(is.na(ALC_DRINKS_DAY_OEB)& wave_id %in% c("W2", "W3") & 
                                                          (ALC_CHECK1 == 96 | ALC_CHECK2 == 2 | ALC_CHECK3 == 4 |ALC_MO_ORD6A == 6) ~ 0, # if people report not having ever tried alcohol they get never instead of NA
                                                        TRUE ~ ALC_DRINKS_DAY_OEB))

# reverse coding
main <- main %>%  mutate(ALC_MO_ORD6A = 7 - ALC_MO_ORD6A)



# DRUGS 
main <- main %>%  mutate(DRUG_OVERCONS_YR_ORD6A = case_when(is.na(DRUG_OVERCONS_YR_ORD6A)& wave_id %in% c("W1", "W2") & 
                                                              (DRUG_CHECK1 == 2 | DRUG_CHECK2 == 2 | DRUG_CHECK3 == 2 |
                                                                 DRUG_CHECK4 == 2  | DRUG_CHECK5 == 2  | DRUG_CHECK6 == 2  | 
                                                                 DRUG_CHECK7 == 2 | DRUG_CHECK8 == 2 | DRUG_CHECK9 == 2 | DRUG_CHECK10 == 2) ~ 1, 
                                                            is.na(DRUG_OVERCONS_YR_ORD6A)& wave_id %in% c("W3") & (DRUG_CHECK11== 2) ~ 1,
                                                            TRUE ~ DRUG_OVERCONS_YR_ORD6A),
                         DRUG_EFFECT_YR_ORD6A = case_when(is.na(DRUG_EFFECT_YR_ORD6A)& wave_id %in% c("W1", "W2") & 
                                                            (DRUG_CHECK1 == 2 | DRUG_CHECK2 == 2 | DRUG_CHECK3 == 2 |
                                                               DRUG_CHECK4 == 2  | DRUG_CHECK5 == 2  | DRUG_CHECK6 == 2  | 
                                                               DRUG_CHECK7 == 2 | DRUG_CHECK8 == 2 | DRUG_CHECK9 == 2 | DRUG_CHECK10 == 2) ~ 1, 
                                                          is.na(DRUG_EFFECT_YR_ORD6A)& wave_id %in% c("W3") & (DRUG_CHECK11== 2) ~ 1,
                                                          TRUE ~ DRUG_EFFECT_YR_ORD6A))


# PROPENSITY
main <- main %>% mutate(ADVENTU_ORD4A = 5-ADVENTU_ORD4A)
...
# >500 cases with missing yobs that cannot be retrieved, using calculated age instead, 
# less missing cases, these cannot be retrieved either
...
main <- main %>%  filter(age %in% c(10:90)) %>% select(-c(MINT,YINT, yob))


#________________FIX VARS: GENDER___________________#

# no missing gender information
...
# dim(risk_info_analyse)  23 rows x  13 cols
