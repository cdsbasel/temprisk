...
files_of_interest <- list.files(path = panel_data_path,
                                recursive = TRUE,
                                pattern =  paste0(unique(na.omit(all_info$varfile)), ".dta"),
                                full.names = TRUE)

dat <- read_dta(files_of_interest) # open file


vars_of_interest <- all_info$origin_varcode # vars of interest

# select vars of interest from the main file
main <- dat %>% select(all_of(vars_of_interest)) %>% 
  mutate(wave_id = paste0("W", year)) %>% 
  select(-year)

# replacing var names
colnames_found <- match(colnames(main), vars_of_interest, nomatch = 0)
colnames(main)[colnames(main) %in% vars_of_interest] <- all_info$varcode[colnames_found]




main <- main %>% mutate_if(is.integer, ~ as.numeric(.x))
# dim(main) 374 rows x 6 cols
...
# Based on study info: baseline (T1: May–June 2013), after 1 year (T2: May–June 2014), and a year thereafter (T3: May–June 2015).


main <- main %>% 
  mutate(date = case_when(wave_id == "W1" ~  as.Date(paste("2013","05","31",sep="-"), "%Y-%m-%d"),
                          wave_id == "W2" ~  as.Date(paste("2014","05","31",sep="-"), "%Y-%m-%d"),
                          wave_id == "W3" ~  as.Date(paste("2015","05","31",sep="-"), "%Y-%m-%d"))) %>% 
  group_by(wave_id) %>% 
  mutate(wave_year =round(mean(year(date)))) %>% ungroup()


#________________FIX YOB & AGE & AGE RANGE___________________#
# Some missing age; but these can be retrieved form previous waves

main <- main %>% 
  group_by(id) %>% 
  fill(age,.direction = 'downup') %>%  # NAs get replaced with neighboring yob for each respondent 
  ungroup() 


# for this panel only keep respondents aged between 10 & 90y.o. + need to recode age (approxmate values)
# (4 = 30-35; 5 = 36–40; 6 = Over 41) based on study information
main <- main %>% 
  mutate( age = case_when(age == 4 ~ 33,
                          age == 5 ~38,
                          age == 6 ~45))

#________________FIX GENDER___________________# 
# Some missing gender information; but cannot retrieve for other waves
...
# reverse code BANL_ALLOC
# 
main <- main %>% mutate(BANK_ALLOC  = 7 - BANK_ALLOC)
