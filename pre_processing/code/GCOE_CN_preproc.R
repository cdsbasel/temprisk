...
main <- main %>% mutate(date = as.Date(as.POSIXct(as.character(date),  format = "%Y%m%d")))
...
# only keep respondents aged between 20 & 90y.o. as per panel documentation
...
inconsistent_dems <- main %>% 
  ...
  summarise(perc_gender = 100*(sum(non_unique_gender)/n()), # ADD RM.NA = T??....+ remove dv variables or checko consistency
