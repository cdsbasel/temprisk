...
panel_data_path <- c("~/Documents/TSRP/Data/HILDA/RawData/") # where the raw panel data is stored
preproc_data_path <- c("~/Documents/TSRP/Data/HILDA/ProcData/") # where to save processed panel data
panel_name <- "HILDA"
...
for ( CurrWave in wave_list) {
  ...
  file_of_interest_name <- unique(na.omit(sub_all_info$varfile)) # file in which the relevant vars are stored
  
  # open file
  file_of_interest <- list.files(path = panel_data_path, pattern = file_of_interest_name, recursive = T, full.names = T)
  wave_data <-read.spss(file_of_interest, to.data.frame = T,use.value.labels = F, reencode = F)
...
# dim(main) 321692 rows x 10 cols
...
main <- main %>% mutate(date = as.Date(INT, "%d/%m/%Y")) %>% select(-INT)
  
main <- main %>% group_by(wave_id) %>% mutate(wave_year = round(mean(year(date)))) %>% ungroup() # calculate the general year for that wave

#________________FIX VARS: AGE RANGE___________________#

# no yob information, only age provided (no NAs)
...
# RISK FIN PROPENSITY
# When RISK_FIN_ORD4A = 5, the person indicates that they have no spare cash to invest, thus not possible to assess risk propensity so we assign NA, RISK_FIN_ORD4B is the follow-up question
main <- main %>% mutate(RISK_FIN_ORD4A = case_when(RISK_FIN_ORD4A == 5 ~ NA_real_,
                                                   TRUE ~ RISK_FIN_ORD4A),
# Just in case, if person answered RISK_FIN_ORD4A (i.e., response != 5), they cannot have an answer for RISK_FIN_ORD4B
                        RISK_FIN_ORD4B = case_when(RISK_FIN_ORD4A %in% c(1:4) ~ NA_real_,
                                                   TRUE ~ RISK_FIN_ORD4B))  %>% 
  # reverse scoring (higher value == higher risk taking)
  mutate(RISK_FIN_ORD4A = 5 - RISK_FIN_ORD4A,
         RISK_FIN_ORD4B = 5 - RISK_FIN_ORD4B)

# SMOKING
# Dependencies: if people report to not smoke they get 0 weekly cigarettes instead of NA
main <- main %>% mutate(CIGS_WK_OEA = case_when(SMK_CHECK %in% c(1, 2)  ~ 0,
                                                CIGS_WK_OEA > 999 ~ NA_real_, # reasonable values, cannot exceed 999 cigarettes/week
                                                           TRUE ~ CIGS_WK_OEA))
...
# dim(risk_info_analyse)  5 rows x  13 cols
