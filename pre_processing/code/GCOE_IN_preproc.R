...
main <- main %>% mutate(date = case_when(!is.na(date) ~  as.Date(as.character(date), "%Y%m%d"),
                                         is.na(date) & wave_id == "2012" ~ as.Date(paste(as.character(YINT), as.character(MINT),as.character(DINT)), "%Y%m%d"),
                                         TRUE ~ as.Date(paste(wave_id,"02","01"), "%Y%m%d"))) %>%  # data collection generally took place between Jan and March each year
  select(-c(DINT, MINT, YINT))
...
# calculate age 
main <- main %>% 
  mutate( yob = case_when(yob > 1989 ~ NA_real_, # different ways to enter yob
                          yob < 99 ~ yob + 1900,
                          TRUE ~ yob),
          age = year(date) - yob) 
...
# only keep respondents aged between 20 & 90y.o. 
...
main <- main %>% 
  mutate_at(vars(CHOICE_LOTTERY_IN_A1:CHOICE_LOTTERY_IN_A8),change_val_lott ) %>%
  rowwise() %>% 
  mutate(LOTTERY_IN_A_RISK = mean(c_across(CHOICE_LOTTERY_IN_A1:CHOICE_LOTTERY_IN_A8), na.rm = FALSE)) %>% 
  ungroup()
...
# dim(risk_info_analyse) 34 rows x 13 cols
