...
panel_data_path <- c("~/Documents/TSRP/Data/TWINLIFE/RawData/") # where the raw panel data is stored
preproc_data_path <- c("~/Documents/TSRP/Data/TWINLIFE/ProcData/") # where to save processed panel data
panel_name <- "TWINLIFE"
...
for (CurrWave in wave_list) {
  ...
  file_of_interest <- list.files(path = panel_data_path, pattern = sub_all_info$varfile, recursive = T) # search relevant file in main data folder
  
  # open the relevant file
  wave_data <-  read_dta(paste0(panel_data_path, file_of_interest))
...
main <- main %>% mutate_if(is.double, ~ as.numeric(.x))
#  dim(main) 44739 rows x 39 cols
...
#________________FIX VARS: RISK PREF. MEASURES (I)___________________#

# DRINKING (I)
# taking into account missing answers because of previous answers (i.e., skipping questions)
main <- main %>%  mutate(ALC_BEER_YR_ORD6A = case_when(is.na(ALC_BEER_YR_ORD6A)  & ALC_CHECK == -81 ~ 6, # if people report not having ever tried alcohol they get never instead of NA
                                                       TRUE ~ ALC_BEER_YR_ORD6A),
                         ALC_WINE_YR_ORD6A = case_when(is.na(ALC_WINE_YR_ORD6A) & ALC_CHECK == -81 ~ 6, # if people report not having ever tried alcohol they get never instead of NA
                                                       TRUE ~ ALC_WINE_YR_ORD6A),
                         ALC_COCKTAIL_YR_ORD6A = case_when(is.na(ALC_COCKTAIL_YR_ORD6A) & ALC_CHECK == -81 ~ 6, # if people report not having ever tried alcohol they get never instead of NA
                                                           TRUE ~ ALC_COCKTAIL_YR_ORD6A))


#________________FIX VARS REPLACING MISSING & NON READABLE RESPONSES: RISK PREF___________________#
main <- main %>% 
...
main <- main %>% mutate(date = as.Date(paste(YINT,MINT,15,sep="-"), "%Y-%m-%d")) # no missing values

main <- main %>% group_by(wave_id) %>% 
  mutate(wave_year = round(mean(year(date)))) %>% ungroup() # calculate the general year for that wave

#________________FIX AGE___________________#

# some missing ages but only one obs can be retrieved successfully

# Retrieve yob from age and interview years
main <- main %>%
  mutate(temp_yob = YINT - age)

# for some years, variables are empty but can be filled/reconstructed from previous or future waves...
main <- main %>%
  group_by(id) %>%
  fill(temp_yob,.direction = 'downup') %>%  # NAs get replaced with neighboring gender for each respondent
  ungroup()

# use yob to calculate MISSING age
main <- main %>%
  mutate(age = case_when(is.na(age) ~ year(date) - temp_yob,
                         TRUE ~ age))
...
main <- main %>%  filter(age %in% c(10:90)) %>% select(-c(MINT,YINT, temp_yob))


#________________FIX VARS: GENDER___________________#

# some missing gender information
...
#  identify respondents with not the same gender reported across the wavesr
main <- main %>%
  group_by(id) %>%
  mutate(non_unique_gender = length(unique(na.omit(gender))) > 1) %>% # some inconsistencies
  ungroup()
...
#________________FIX VARS: RISK PREF. MEASURES (II)___________________#

# DRINKING (II)
# reverse scoring
main <- main %>%  mutate(ALC_BEER_YR_ORD6A = 7 - ALC_BEER_YR_ORD6A,
                         ALC_WINE_YR_ORD6A = 7 - ALC_WINE_YR_ORD6A,
                         ALC_COCKTAIL_YR_ORD6A = 7 - ALC_COCKTAIL_YR_ORD6A)


# AGGRESSIVE / DELIQUENT BEHAVIOUR
# taking into account missing answers because of previous answers (i.e., skipping questions)
main <- main %>%  mutate(DODGE_FARE_YR_OEA = case_when(is.na(DODGE_FARE_YR_OEA) & DODGE_FARE_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                       TRUE ~ DODGE_FARE_YR_OEA),
                         STAYOUT_NIGHT_YR_OEA = case_when(is.na(STAYOUT_NIGHT_YR_OEA) & STAYOUT_NIGHT_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                          TRUE ~ STAYOUT_NIGHT_YR_OEA),
                         SKIP_SCHOOL_YR_OEA = case_when(is.na(SKIP_SCHOOL_YR_OEA) & SKIP_SCHOOL_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                        TRUE ~ SKIP_SCHOOL_YR_OEA),
                         SHOPLIFT_YR_OEA = case_when(is.na(SHOPLIFT_YR_OEA) & SHOPLIFT_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                     TRUE ~ SHOPLIFT_YR_OEA),
                         STEAL_PEOPLE_YR_OEA = case_when(is.na(STEAL_PEOPLE_YR_OEA) & STEAL_PEOPLE_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                         TRUE ~ STEAL_PEOPLE_YR_OEA),
                         GRAFFITI_YR_OEA = case_when(is.na(GRAFFITI_YR_OEA) & GRAFFITI_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                     TRUE ~ GRAFFITI_YR_OEA),
                         VANDALISE_YR_OEA = case_when(is.na(VANDALISE_YR_OEA) & VANDALISE_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                      TRUE ~ VANDALISE_YR_OEA),
                         CYBERBULLY_YR_OEA = case_when(is.na(CYBERBULLY_YR_OEA) & CYBERBULLY_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                       TRUE ~ CYBERBULLY_YR_OEA),
                         ILLEGAL_DL_YR_OEA = case_when(is.na(ILLEGAL_DL_YR_OEA) & ILLEGAL_DL_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                       ILLEGAL_DL_YR_OEA > 10000 ~ NA_real_, # some unrealistic values in the data (i.e., 3 instances of 99999999)
                                                       TRUE ~ ILLEGAL_DL_YR_OEA),
                         BULLY_CLASS_YR_OEA = case_when(is.na(BULLY_CLASS_YR_OEA) & BULLY_CLASS_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                        TRUE ~ BULLY_CLASS_YR_OEA),
                         THREATEN_ROB_YR_OEA = case_when(is.na(THREATEN_ROB_YR_OEA) & THREATEN_ROB_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                         TRUE ~ THREATEN_ROB_YR_OEA),
                         FIGHT_PHYS_YR_OEB = case_when(is.na(FIGHT_PHYS_YR_OEB) & FIGHT_PHYS_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                       TRUE ~ FIGHT_PHYS_YR_OEB),
                         DRIVE_WO_LICENSE_YR_OEA = case_when(is.na(DRIVE_WO_LICENSE_YR_OEA) & DRIVE_WO_LICENSE_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                             TRUE ~ DRIVE_WO_LICENSE_YR_OEA),
                         DRIVE_ALC_YR_OEA = case_when(is.na(DRIVE_ALC_YR_OEA) & DRIVE_ALC_CHECK == 2 ~ 0, # if people report not having ever they get 0  instead of NA
                                                      TRUE ~ DRIVE_ALC_YR_OEA))
...
# dim(risk_info_analyse)  33 rows x  13 cols
