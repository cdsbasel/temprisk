...
for (CurrWave in wave_list) {
  ...
  for (CurrFile in file_list)  {
    ...
    folder_of_interest <- unique(file_info$varfolder)
    
    vars_of_interest <- file_info$origin_varcode # vars of interest
    
    # open file 
    data <- read_csv(paste0(panel_data_path, CurrWave, "/", folder_of_interest, "/", CurrFile), show_col_types = FALSE)
    ...
    # fixing ids for retest sample (added 1)
    if (CurrWave %in% c("retest_basel","retest_berlin")) {
      data <- data %>% mutate(id = as.numeric(id) -1)
    }
...
main <- main %>% mutate(date = case_when(wave_id == "main" ~ as.Date(paste("2015","06","15",sep="-"), "%Y-%m-%d"),
                                         wave_id == "retest_basel" ~ as.Date(paste("2015","09","15",sep="-"), "%Y-%m-%d"),   # fake date --> 3 months later
                                         wave_id == "retest_berlin" ~ as.Date(paste("2015","12","15",sep="-"), "%Y-%m-%d")))   # fake date --> 6 months later

main <- main %>% group_by(wave_id) %>% mutate(wave_year = round(mean(year(date)))) %>% ungroup() # calculate the general year for that wave

#________________FIX VARS: WAVE ID ___________________#
main <- main %>% mutate(wave_id = case_when(wave_id %in% c("retest_basel","retest_berlin") ~ "W2", # combine Basel and Berlin data together
                                            wave_id == "main" ~ "W1"))
...
main <- main %>% mutate(gender = case_when(
  gender == "male" ~ 0,
  gender == "female" ~ 1))   

#  identify respondents with not the same gender reported across waves 
main <- main %>%
  group_by(id) %>%
