...
#Script to plot the meta-analytic correlation matrix (corrected for attenuation). included in companion website
...
ma <- list(measure = read_csv(paste0(data_path, "cor_mat_convergent_measure_dat_corrected.csv")),
           domain = read_csv(paste0(data_path, "cor_mat_convergent_domain_dat_corrected.csv")))
...
p1 <- ggplot(data = melted_cormatB, aes(x, y, fill = (estimate_corr)))+
...
p2 <- ggplot(data = melted_cormat, aes(x = y, y = x, fill = estimate_corr))+
  geom_tile(color = "grey50", linewidth = .2)+
  geom_richtext(lineheight = .75, family = "Source Sans 3", 
                fill = NA, label.color = NA,fontface = "bold",
                aes(color = lbl_color, label = pooled_est_lbl), 
...
ggsave( filename = paste0(output_path,"cor_matrix_fig_correction.png"),
       plot = p,
       height = 30, 
       width = 45,
       units = "cm",
