...
for (curr_yr in c(5,10,20)) {
  ...
  dt <- fread(paste0(data_path,"complete_agg_retest_yb", curr_yr, ".csv"))
  ...
  dt <- dt %>% 
    filter(age_group != "10-90" &
              ...
              rho_val == .5 &
              cor_metric == "pearson") %>% 
    group_by(age_group, min_n, month_bin, age_bin) %>% 
    summarise(n_cor = n()) %>% 
    mutate(year_age_group = paste0(age_bin,"-year bins"),
              min_n = paste0("Min n: ", min_n),
              month = paste0(month_bin,"-month bins"))
...
p <- dat_thrshA %>% 
  ...
  scale_x_continuous(limits = c(0,12750),
                     expand = c(0.01,0.01),
...
ggsave(filename = paste0(output_path,"threshold_retest_plot.png"), plot = p, height = 15, width = 25, units = "cm", dpi = 600)
...
# AGGREGATED RETEST CORRELATIONS ---------------------------------------------------
...
dat_agg$month <- factor(dat_agg$month, levels = c("3-month bins",  "6-month bins", "12-month bins"))

dat_aggB$year_age_group <- factor(dat_aggB$year_age_group, levels = c("5-year bins",  "10-year bins", "20-year bins"))
dat_aggB$month <- factor(dat_aggB$month, levels = c("3-month bins",  "6-month bins", "12-month bins"))
...
p <- dat_agg %>% 
  ...
  labs(x = "Number of aggregated correlations",
       y = "",
       fill = "") +
  facet_grid(year_age_group~month, scales = "free_y",space = "free_y", switch = "y") +
...
ggsave(filename = paste0(output_path,"threshold_retest_agg_plot.png"), plot = p, height = 15, width = 30, units = "cm", dpi = 600)
