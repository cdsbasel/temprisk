...
# Script that plots the results of the main Variance Decomposition analysis.
...
t <- read_csv(paste0(data_path,"summary_shapley_values_retest.csv")) %>% 
  filter(measure_category == "Omnibus")

boot_t <-  read_csv(paste0(data_path,"summary_shapley_values_retest_boot.csv")) %>% 
  filter(measure_category == "Omnibus")
...
p <- p_omni /p_categ + plot_layout(heights = c(0.75,1))

ggsave(plot = p, filename = paste0(output_path, "shapley_decomp_retest_fig.png"),
       dpi = 300, width = 25, height = 22, units = "cm")
