...
# The  dataframes with the retest information (one with and another without the subject ids) are saved as csv + RDS files. Prior to saving
# the output we check if retest correlations specificx to the propensity item were asked.
...
# PATH INFORMATION ---------------------------------------------------
preproc_data_path <- c("~/Documents/TSRP/Data/ANPS/ProcData/") # location of pre-processed panel data
retest_data_wid_path <- c("~/Documents/TSRP/Data/ANPS/ProcData/") # retest data with ids storage path
...
countries <- sort(unique(proc_data$country))
for (CurrCountry in countries) {
  
  
  proc_data_country <- proc_data %>% filter(country == CurrCountry)
  
  panel_country <- unique(proc_data_country$panel)
  # How many unique SURVEY waves are in the data? SORT IN ORDER!
  waves <- sort(unique(proc_data_country$wave_id))
  
  
  #_________________ GET PANEL INFO _________________________#
  
  panel_info <- proc_data_country %>% 
    select(c(continent, country, language:sample_type)) %>% distinct() %>% 
    mutate(sample = panel_country)
  retest2_df <- NULL
  retest_df <- NULL
  ...
  # check if retest correlations were computed for the propensity measure
  if (nrow(filter(retest_df, measure_category == "pro")) > 0) {
    
    write_csv(retest_df, file = paste0(retest_data_path,panel_country,"_retest_data.csv"))
    write_rds(retest2_df, file = paste0(retest_data_wid_path,panel_country,"_retest_data.rds"))
