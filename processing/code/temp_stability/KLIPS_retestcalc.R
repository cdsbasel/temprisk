...
# PATH INFORMATION ---------------------------------------------------
preproc_data_path <- c("~/Documents/TSRP/Data/KLIPS/ProcData/") # location of pre-processed panel data
retest_data_wid_path <-c("~/Documents/TSRP/Data/KLIPS/ProcData/") # retest data with ids storage path
...
panel_info <- proc_data %>% select(c(continent:sample_type)) %>% distinct() %>% mutate(sample = sample)
panel <- "KLIPS"
# COMPUTING CORRELATIONS & SAVING OUTPUT --------------------------------------------

# How many unique SURVEY waves are in the data? SORT IN ORDER!
# waves <- sort(unique(proc_data$wave_id))
waves <- c("W7", "W10", "W23", "W24", "W25") 
retest2_df <- NULL
retest_df <- NULL
...
print(paste0(sample, " retest correlation computation done!"))
