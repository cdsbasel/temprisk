...
# PATH INFORMATION ---------------------------------------------------
preproc_data_path <- c("~/Documents/TSRP/Data/GLES/ProcData/") # location of pre-processed panel data
retest_data_wid_path <- c("~/Documents/TSRP/Data/GLES/ProcData/") # retest data with ids storage path
...
sampls <- sort(unique(proc_data$panel))
for (CurrSampl in sampls) {
  
  # select sample data
  panel_sample <- CurrSampl
  sample_proc_data <- proc_data %>% filter(panel == CurrSampl)
  
  #How many unique SURVEY waves are in the data? SORT IN ORDER!
  # earliest to latest wave (wave# != dates), avoiding negative time difference calculations
  waves_sort <- sample_proc_data %>% group_by(wave_id) %>% summarise(mean_date = mean(date)) %>% arrange(mean_date)
  waves <- waves_sort$wave_id
  
  
  #_________________ GET PANEL INFO _________________________#
  
  panel_info <- sample_proc_data %>% 
    select(c(continent:sample_type)) %>% distinct() %>% 
    mutate(sample = panel_sample )
  ...
  write_csv(retest_df, file = paste0(retest_data_path,panel_sample,"_retest_data.csv"))
  write_rds(retest2_df, file = paste0(retest_data_wid_path,panel_sample,"_retest_data.rds"))
} # sample loop
