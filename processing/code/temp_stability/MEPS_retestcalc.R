...
# PATH INFORMATION ---------------------------------------------------
preproc_data_path <- c("~/Documents/TSRP/Data/MEPS/ProcData/") # location of pre-processed panel data
retest_data_wid_path <-c("~/Documents/TSRP/Data/MEPS/ProcData/") # retest data with ids storage path
...
waves <- unique(proc_data$wave_id)

# waves <- c("H106_2005", "H106_2006", "H114_2006", "H114_2007", "H122_2007", "H122_2008","H130_2008",
# "H130_2009", "H139_2009", "H139_2010", "H148_2010", "H148_2011", "H156_2011", "H156_2012",
# "H164_2012", "H164_2013", "H172_2013", "H172_2014", "H183_2014", "H183_2015", "H193_2015",
# "H193_2016", "H202_2016", "H202_2017", "H65_2000",  "H65_2001",  "H71_2001",  "H71_2002", 
# "H80_2002",  "H80_2003",  "H86_2003",  "H86_2004",  "H98_2004",  "H98_2005") 
# time consuming to run, only retest calculations are possible between waves with the same prefix

wave_pairs <- split(waves, ceiling(seq_along(waves)/2))
retest2_df <- NULL
retest_df <- NULL
...
for (CurrAgeBin in unique(age_groups$age_bin_categ)) {
  ...
  #_________________WAVE PAIRS _________________________#
  
  
  for (CurrPairNum in 1:length(wave_pairs)) {
    
    CurrPair <- wave_pairs[[CurrPairNum]]
    
    proc_data_pair <- proc_data %>% 
      filter(wave_id %in% CurrPair)
    
    waves_p <- sort(unique(proc_data_pair$wave_id))
    ...
    retest <- left_join(retest, risk_info, by = "varcode") %>% 
      bind_cols(panel_info) %>%
      relocate(sample, .after = panel)
    
    retest2 <- results[[2]] # with ids
    retest2 <- left_join(retest2, risk_info, by = "varcode") %>% 
      bind_cols(panel_info) %>%
      relocate(sample, .after = panel)
...
write_csv(retest_df, file = paste0(retest_data_path,panel,"_retest_data.csv"))
write_rds(retest2_df, file = paste0(retest_data_wid_path,panel,"_retest_data.rds"))
