...
# PATH INFORMATION ---------------------------------------------------
preproc_data_path <- c("~/Documents/TSRP/Data/SOEP/ProcData/") # location of pre-processed panel data
retest_data_wid_path <-c("~/Documents/TSRP/Data/SOEP/ProcData/") # retest data with ids storage path
...
waves_sort <- proc_data %>% group_by(wave_id) %>% summarise(mean_date = mean(date)) %>% arrange(mean_date)
waves <- waves_sort$wave_id
# c("1998" ,"2002" ,"2004","2006","2008","2009","2010" ,"2011" ,   
#   "2012" ,"2013" ,"2014", "2015","2016" ,"2017", "2018" ,"2019",    
#     "2020_cov", "2020", "2021_cov")
...
## there is a substantial overlap between the covid and core survey for 2020. 
## One average the 2020 core survey was completed after the cov_2020, but for
##  some of the correlation the mean/median time difference is negative:
##  the correlations for these pairs of waves are removed
retest_df <- retest_df %>% filter(!(wave_id_t1 ==  "2020_cov" &  wave_id_t2 ==  "2020"))
retest2_df <- retest2_df %>%  filter(!(wave_id_t1 ==  "2020_cov" &  wave_id_t2 ==  "2020"))
