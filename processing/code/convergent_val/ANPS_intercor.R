...
intercor_data_wid_path <-  c("~/Documents/TSRP/Data/ANPS/ProcData/") # intercor data with ids storage path
...
for (CurrCountry in countries) {
  ...
  varcodes <- crossing(var_a = unique(proc_data_country$varcode),
                       var_b = unique(proc_data_country$varcode)) %>% 
    filter(var_a != var_b)
  
  risk_info_a <- risk_info %>% filter(varcode %in% varcodes$var_a)
  ...
  for (CurrAgeBin in unique(age_groups$age_bin_categ)) {
    ...
    write_csv(intercor_df, file = paste0(intercor_data_path,panel_country,"_intercor_data.csv"))
    write_rds(intercor2_df, file = paste0(intercor_data_wid_path,panel_country,"_intercor_data.rds"))
...
print(paste0(panel, " intercor correlation computation done!"))
