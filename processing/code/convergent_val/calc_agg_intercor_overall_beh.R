...
# In this script we recoded the domain of behavioural measures, such that instead of 4 domains there is a single domain- "Financial"
...
dt <- data %>% 
  ...
  mutate(cor_pearson = case_when(cor_pearson > 0.99 ~ 0.99,
         ...
         icc2_1_log = case_when(icc2_1_log > 0.99 ~ 0.99, 
                                TRUE ~ icc2_1_log)) %>% 
  # give behavioral measures all the same domain label
  mutate(domain_name_a = case_when(measure_category_a == "beh" ~ "fin", TRUE ~ domain_name_a),
         domain_name_b = case_when(measure_category_b == "beh" ~ "fin", TRUE ~ domain_name_b)) %>% 
  # creating labels
...
write_csv(wcor_df_simple2, paste0(output_data_path,"complete_agg_overall_beh_intercor_yb10.csv"))
# wcor_df_simple2 <- wcor_df_simple %>% filter(age_bin == 20)
# write_csv(wcor_df_simple2, paste0(output_data_path,"complete_agg_overall_beh_intercor_yb20.csv"))
# wcor_df_simple2 <- wcor_df_simple %>% filter(age_bin == 5)
# write_csv(wcor_df_simple2, paste0(output_data_path,"complete_agg_overall_beh_intercor_yb5.csv"))
print("done!")
